...
config['proxy-groups'] = [
  {
    ...
    name: '默认节点',
    type: 'select',
    proxies: uniqueList(['台湾省', ...proxyGroupsRegionNames, '直连']),
  ...
  // ---- 场景策略组（按需追加） ----
  // 统一生成场景策略组的 proxies（优先项可通过 preferred 数组传入）
  function makeSceneProxies(preferred = []) {
    return uniqueList([...preferred, '自动测速', '默认节点', ...proxyGroupsRegionNames, '直连'])
  }
...
(function reorderProxyGroups() {
  ...
  const regionNameRegex = /(台湾省|日本|美国|新加坡|香港|韩国|中国大陆|英国|德国|马来西亚|土耳其)/;
